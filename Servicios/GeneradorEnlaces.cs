...
namespace WebApiAutores.Servicios
{
    public class GeneradorEnlaces
    {
        ...
        public async Task GenerarEnlaces(AutorDTO autorDTO, string version)
        {
            var esAdmin = await EsAdmin();
            var Url = ConstruirURLHelper();

            autorDTO.Enlaces.Add(new DatoHATEOAS( enlace: Url.Link("obtenerAutor"+version, new { id = autorDTO.Id }), descripcion: "self", metodo: "GET"));

            if (esAdmin)
            {
                autorDTO.Enlaces.Add(new DatoHATEOAS( enlace: Url.Link("actualizarAutor" + version, new { id = autorDTO.Id }), descripcion: "autor-actualizar", metodo: "PUT"));

                autorDTO.Enlaces.Add(new DatoHATEOAS( enlace: Url.Link("borrarAutor" + version, new { id = autorDTO.Id }), descripcion: "self", metodo: "DELETE"));
            }
